    ...
    return this.request(needed)
        .then(highlighted => {
            needed.forEach(block => {
                ...
                let found = highlighted.find(b => block.id === b.id);
...
Torchlight.prototype.request = function (blocks) {
    ...
    let host = this.config('host', 'https://api.torchlight.dev');
...
Torchlight.prototype.fan = function (blocks) {
    let highlighted = [];
    let errors = [];
    let requests = chunk(blocks, this.chunkSize).map(chunk => this.request(chunk));
